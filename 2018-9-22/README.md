...
将 $N$ 个皇后放在 $N\times N$ 的棋盘上，互相不侵犯（不在同行、同列、同45度斜线上）。问有多少种摆法。

![](http://poj.org/images/3239_1.png)

> 将什么作为一种状态？

若将放置着棋子的棋盘( $N\times N$ )作为状态，则要储存的状态较大，状态转移要做的判断比较多。  
...
```
延伸：$N$ 的范围到300时，求一种构造棋盘的方案。[link](http://poj.org/problem?id=3239&lang=zh-CN)
## 什么是深度优先搜索/广度优先搜索
